...
pkgInst <- which(!(pkgReq %in% pkgList))
if (length(pkgInst) > 0) {
	for (i in 1:length(pkgInst)) install.packages(pkgReq[pkgInst[i]])
}
...
path <- "."
#path <- "/srv/shiny-server/stdmxmnd_inhibition"
sourceCpp(file.path(path, "kernel/psokernel.cpp"))
...
shinyServer(function(input, output, session) {
	...
	appMsg <- reactiveValues(errMsg = NULL, result = NULL, applog = NULL, cpustatus = 0, checkinput = 0)
	...
	COLLECTINPUT <- reactive({
		...
		appInput$psoOptions <- rbind(
			...
			c(4, 0, 1, 			 1,        256,        200, 1e-6,        2,        2,        2,      1.2,      0.2,      0.8, 1, 0)  # ASSIST
		)
	...
	# The Main Function
	DOSEARCH <- reactive({
		if ((appInput$designInfo[1] > 0) & (appMsg$cpustatus == 1)) {
	...
	output$result <- renderPrint({
		if ((appInput$designInfo[1] > 0) & (!is.null(appOutput$pout))) {
			...
			if (all((pUB - pLB) == 0)) {
				paraMsg <- paste0("Parameters: Vmax = ", pUB[1], "; ", paste0(paste0(inhiInfo$PARNAME, " = ", pUB[-1]), collapse = "; "))
				designName <- "Locally "
			} else {
				paraMsg <- paste0("Parameter Space: Vmax = ", pUB[1], "; ", 
			...
			if (is.null(appMsg$errMsg)) {
				...
				#cat("\n")
				#if (all((pUB - pLB) == 0)) {
				#	cat(paste0("Best Design Criterion Value: ", round(appOutput$pout$fGBest, 4), "\n"))
				#} else {
				#	cat(paste0("Best min. efficiency: ", round(exp(appOutput$pout$fGBest)*100, 4), " %\n"))
				#}
				cat("\n")
				cat(paste0("Maximal Directional Derivative Function Value: ", round(appOutput$dout$maxVal, 4), "\n"))
				cat(paste0("Atwood's D-efficiency Bound: ", 
					formatC(round(100*length(pLB)/(length(pLB) + appOutput$dout$maxVal), 2), format = "f", digits = 2), "%\n"))
				cat(paste0("Pazmen's D-efficiency Bound: ", 
					formatC(round(100*exp(-appOutput$dout$maxVal/length(pLB)), 2), format = "f", digits = 2), "%\n"))
				cat("\n")
	...
	# 	}
	# 	dev.off()
	# 	list(src = outfile, contentType = 'image/png', width = 400, height = 400, alt = "")
	# }, deleteFile = TRUE)

	output$outcome <- renderImage({

		outfile <- tempfile(fileext='2.png')
		png(outfile, width=440, height=400)
		...
		if ((appInput$designInfo[1] > 0) & (!is.null(appOutput$dout))) {
			...
			contour(appOutput$dout$sGrid, appOutput$dout$iGrid, appOutput$dout$dispVal, add = TRUE, col = "gray90", labcex = 1.2)
		...
		dev.off()
		list(src = outfile, contentType = 'image/png', width = 440, height = 400, alt = "")
	}, deleteFile = TRUE)
